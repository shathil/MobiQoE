...
import com.qoeapps.qoenforce.sysmeta.SystemInformaiton;
...
public class SessionHandler {
	...
	//public static final String tcpLocation = "/proc/net/tcp";
	//public static final String udpLocation = "/proc/net/udp";
	//public static final String tcp6Location = "/proc/net/tcp6";
	//public static final String udp6Location = "/proc/net/udp6";





	private String CLASS_NAME = SessionHandler.this.getClass().getSimpleName();
	...
	private void handleTCPPacket(byte[] clientpacketdata, IPv4Header ipheader, TCPHeader tcpheader){
        ...
        */
		...
		if(tcpheader.isSYN()){
        	...
        	if(session == null){
        		//Log.d(CLASS_NAME,"**** ==> Session not found: "+
        	...
        	if(tcpheader.isPSH()){
        		...
        		//ack it
        		//Log.d(CLASS_NAME,"FIN from vpn client, will ack it.");
			...
			//Log.d(CLASS_NAME,"**** Reset client connection for dest: "+PacketUtil.intToIPAddress(ipheader.getDestinationIP())+":"+tcpheader.getDestinationPort()
        	//		+"::"+PacketUtil.intToIPAddress(ipheader.getSourceIP())+":"+tcpheader.getSourcePort());
        	resetConnection(ipheader, tcpheader);
        }else{
        	//Log.d(CLASS_NAME,"unknown TCP flag");
        	//String str1 = PacketUtil.getOutput(ipheader, tcpheader, clientpacketdata);
            //Log.d(CLASS_NAME,">>>>>>>> Received from client <<<<<<<<<<");
            //Log.d(CLASS_NAME,str1);
            //Log.d(CLASS_NAME,">>>>>>>>>>>>>>>>>>>end receiving from client>>>>>>>>>>>>>>>>>>>>>");
        }
	}
	/**
	...
	public void handlePacket(byte[] data, int length) throws PacketHeaderException{
		...
		if(ipheader.getIpVersion() != 4 || (ipheader.getProtocol() != 6 && ipheader.getProtocol() != 17)){
			...
			if(ipheader.getIpVersion() != 4){
				Log.e(CLASS_NAME,"********===> Unsupported IP Version: "+ipheader.getIpVersion());
			}
			if(ipheader.getProtocol() != 6 && ipheader.getProtocol() != 17){
				Log.e(CLASS_NAME,"******===> Unsupported protocol: "+ipheader.getProtocol());
			}*/
			return;
		}
		...
		if(ipheader.getProtocol() == 6){
			...
			//TODO: Hoque -- perform local table lookup and then check the proc file
